define(function (require) {
    ...
    return {
        ...
        globalVars: {
            ...
            pageName: '', // 页面实体名称
            entityName: '', // 实体名称
            disableRow: null, //停用院系时暂存的院系信息
            disableTable: null,
            rollbackEnableFlag: false//标记取消填写停用原因导致的  启用状态0->1变化
        },
        /**
         * 页面被重新激活时调用
         */
        ...
        pageCreated: function () {
            ...
            this.globalVars.entityName = '院系';
            this.globalVars.pageName = '院系';
        },
        /**
        ...
        action_show_dialog: function (event) {
            ...
            if (targetBtn.id == 'addBtn') {
                ...
                this.nextOrder("college", "jcsjgl").then(nextOrder => {
                    self.$setVal(formId, {"qyzt": "1","px": nextOrder, "sfpydw": "1", "sfjxdw": "1" });
                });
        ...
        saveAndAddBtn_click: function (event, closeNext) {
            let funcCode = 'api_add';
            this.misPage('modelForm')
                .validateFormValue()
                .then((value) => {
                    if (!value) {
                    ...
                    this.misRequest(funcCode, params, event.btn).then((data) => {
                        if (data.code == '0') {
                            this.misPage('modelAdvTable').reloadData();
                            this.$page('modelForm').resetFields();
                            this.$Msg(this.$LANG('singleModelMessageSaveSuccess'), 'success');
                        }
                    });
                });
        ...
        form_item_change: function (event) {
            ...
            if (arg[0] === "qyzt") {
                if (arg[1] === "0") {
                     this.misPage('modelForm').setHide({ 'tyyy': false });
                    this.globalVars.dialogVm.$setModelConfig('modelForm',{"tyyy":{"form.required":true}});
                } else {
                    this.misPage('modelForm').setHide({ 'tyyy': true });
                    this.globalVars.dialogVm.$setModelConfig('modelForm',{"tyyy":{"form.required":false}});
        ...
        * 切换状态
        */
        toggleEnableStatus: function (row, table) {
            console.log('aa')
            if (row['qyzt'] == '1') {
                //如果是回滚状态导致的启用状态修改，不做处理
                if(this.globalVars.rollbackEnableFlag){
                    this.globalVars.rollbackEnableFlag = false;
                    return;
                }
                //清空停用原因
                row['tyyy'] = '';
                let params = {
                    misRequestParams: { errorTitle: '操作失败' },
                    params: { ...row, forceUpdateProps: 'tyyy' }
                };
                this.misRequest('api_upd', params).then((data) => {
                    if (data.code == '0') {
                ...
                this.globalVars.disableRow = row;
                this.globalVars.disableTable = table;
                let dialog = this.$pageDialog({ key: 'disableReasonDialog' });
                dialog.$setModelConfig('disableReasonfield',{"tyyy":{"form.required":true}});
            }
         ...
         * 填写停用原因后保存院系信息
         */
        disableAndSave: function (event) {
             this.misPage('disableReasonfield')
                .validateFormValue()
                .then((value) => {
                    if (!value) {
                        //校验不通过为false
                   ...
                   let row = this.globalVars.disableRow;
                   row['tyyy'] = value.tyyy;
                   let params = {
                        misRequestParams: { errorTitle: '操作失败' },
                        params: { ...row, forceUpdateProps: 'tyyy' }
                    };
                    this.misRequest('api_upd', params, event.btn).then((data) => {
                        if (data.code == '0') { 
                            this.misPage('disableReasonDialog').closeWindow();
                            this.$Msg('操作成功', 'success');
                        }
                    });
                });
         ...
         * 点击停用后取消，刷新表格
         */
        cancelDisable: function (event) {
            this.globalVars.rollbackEnableFlag = true;
            this.misPage('disableReasonDialog').closeWindow();
         ...
         * 关闭弹窗时触发的逻辑
         */
        closeDisableWindowToDo: function(){
            this.globalVars.disableTable.reloadData();
        },
        /**
         * 设置已选*条
         */
