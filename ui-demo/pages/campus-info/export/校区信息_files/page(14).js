define(function (require) {
  ...
  return {
    ...
    pageCreated: function () {
      ...
      this.$request("eaxjydgl_xsd_ckydxz").then(res => {
        if (res.data.code === "0") {
    ...
    getCardList: function () {
      // 获取异动申请卡片 
      this.$request("eaxjydgl_xsd_xssqyddllb_xsd_cxlb").then(res => {
        if (res.data.code === "0") {
    ...
    handleClickJump: function (value, isOpenDialog = true) {
      // 学生申请提示信息 eaxjydgl_xsd_xssqtsxx 
      this.globalVars.currApply = value
      this.misRequest('eaxjydgl_xsd_xssqtsxx', {
          ...
          if (!res.data.modelData.ignore || !isOpenDialog) {
            ...
            window.$resUserNotice({
              ...
              onConfirm: () => {
                if (isOpenDialog) {
                  const dialogParams = {
                    auditFormPageCode: 'eaxjydgl' + value.dm + 'Form',
                    ydlxid: value.id,
                    sfdsq: '0',
                    showAuditLog: false,
    ...
    openYDInfoDialog: function (event) {
      ...
      this.misRequest('eaxjydgl_cxdtsqxx', {
        params: {
          id: event.row.id
        }
      }).then(res => {
      ...
      if (event.row.approvalStatus === '0') {
        // 草稿
        dialogParams = {
          auditFormPageCode: 'eaxjydgl' + event.row.ydlxdm + 'Form',
      ...
      // 查询是否需要展示“打印”按钮
      this.misRequest('eaxjydgl_xsxqxsydbb', {
        params: {
          id: event.row.id
        }
      }).then(res => {
    ...
    action_ev_rzpz94ir: function (event, closeNext) {
      this.misRequest('eaxjydgl_dyydbb', {
        params: {
          params: {
